jQuery(function() {
  jQuery('form.as_form').live('submit', function(e){
    var as_form = jQuery(this).closest("form");
    $.ajax({
      url: as_form.attr('action'),
      type: as_form.attr('method'),
      dataType: 'script',
  ...
  jQuery('a.as_action').live('click', function(e){
    var action_link = ActiveScaffold.ActionLink.get(jQuery(this));
    var template = 'script';
    var action = action_link.action;
    if(action == 'new' || action == 'edit' || action == 'show' || action == 'show_search'){
      template = 'html';
    }
    $.ajax({
      url: action_link.url,
      dataType: template,
      type: action_link.method,
      ...
      beforeSend: function(xhr, settings){
        if (action_link) {
          if (action_link.is_disabled()) {
            ...
            if (action_link.loading_indicator)
              action_link.loading_indicator.css('visibility','visible');
            action_link.disable();
          }
        }
        return true;
  ...
  jQuery('a.as_cancel').live('click', function(e){
  ...
  jQuery('a.as_paginate').live('click', function(e){
    var as_paginate = jQuery(this);
    $.ajax({
      url: as_paginate.attr('href'),
      dataType: 'script',
      type: 'get',
      ...
      success: function(event, status, xhr){
        as_paginate.prevAll('img.loading-indicator').css('visibility','hidden');
        return true;
      },

      error: function(xhr, status, errorThrown){
        var as_scaffold = as_paginate.closest('.active-scaffold');
